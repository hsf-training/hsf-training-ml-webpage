...
> The MSE loss function is a special case of the using the maximum likelihood method when the model $$f$$ describes measurements as outcomes of independent Gaussian random variables where each random variable has the same standard deviation. In this case $$\ln L(\theta) = \text{const.} - \frac{1}{2\sigma^2} \sum_{i=1}^N (y_i-f(x_i, \theta))^2$$ where $$L$$ is the likelihood function, $$(x_i, y_i)$$ is the data, $$\sigma$$ is the standard deviation of the random variables, and $$\theta$$ represents the model parameters. Thus minimizing the MSE loss function is equivalent to maximizing the likelihood.
{: .callout}
...
In general, a model with more parameters can fit a more complicated data set. Consider the plot above
